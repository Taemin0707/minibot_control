...
from pedestrian_tracking.msg import BoundingBoxes, ObjectCount
from pedestrian_tracking.msg import PedestrianInfo, PedestrianInfos

class PedestrianTracker(object):
    """보행자 인식 결과를 바탕으로 보행자들의 위치와 가장 가까운 보행자와의 거리를 반환한다."""
    ...
    def update_object_count(self, data):
            ...
            try:
                for i in range(len(bounding_boxes.bounding_boxes)):
                    if bounding_boxes.bounding_boxes[i].Class == 'person':
                        ...
                        pedestrain_marker.header.frame_id = 'camera'
                        pedestrain_marker.header.stamp = rospy.Time()
                        pedestrain_marker.id = int(_id)
                        ...
                        pedestrain_marker.pose.position.x = float(depth_point[2]) 
                        pedestrain_marker.pose.position.y = float(depth_point[0]) * -1
                        pedestrain_marker.pose.position.z = 0
                        pedestrain_marker.lifetime = rospy.Duration.from_sec(0.5)
                        pedestrain_markers.markers.append(pedestrain_marker)
            except:
        ...
        try:
            ...
            #self.marker_pub.publish(pedestrain_markers)
